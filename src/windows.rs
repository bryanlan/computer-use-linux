...
pub const GNOME_SHELL_EXTENSION_SERVICE: &str = "dev.avifenesh.ComputerUseLinux.WindowControl";
pub const GNOME_SHELL_EXTENSION_OBJECT_PATH: &str = "/dev/avifenesh/ComputerUseLinux/WindowControl";
pub const WINDOW_PERMISSION_HINT: &str = "Computer Use could not access a GNOME window list backend. Targeted window input requires session-bus access plus either GNOME Shell Introspect permission or the Linux Computer Use GNOME Shell extension backend. Run setup_window_targeting to install the extension backend.";
    ...
    match list_extension_windows().await {
        ...
        Err(extension_error) => match list_gnome_shell_introspect_windows().await {
            ...
            Err(introspect_error) => Err(anyhow::anyhow!(
                "Linux Computer Use GNOME Shell extension failed: {extension_error:#}; GNOME Shell Introspect failed: {introspect_error:#}"
            )),
        },
    }
}
...
fn ensure_backend_can_focus_target(target: &WindowTarget, window: &WindowInfo) -> Result<()> {
    if target.requires_exact_focus() && window.backend != GNOME_SHELL_EXTENSION_BACKEND {
        bail!(
            "Exact window targeting requires the Linux Computer Use GNOME Shell extension backend; {} can list the matched window but cannot activate a specific window safely.",
            window.backend
        );
    }
    Ok(())
...
async fn activate_extension_window(window_id: u64) -> Result<()> {
    ...
    if ok {
        ...
        bail!("Linux Computer Use GNOME Shell extension refused activation: {message}");
    }
}
